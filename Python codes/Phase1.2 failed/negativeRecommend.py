...
"""
Compute [Negative related items] matrix from [two dimensions rating matrix] format matrix
"""


def calc_negative_mat_from_rating_mat(path):
    ...
    # Build matrix for NI.
    ni = np.zeros((columns, columns))

    # Compute N set for every user.
    for u in range(1, rows):
        ...
        for i in range(1, columns):
            if rating_matrix[u, i] < avg and rating_matrix[u, i] != 0:
                sub_set.append(i)

        for i in range(0, len(sub_set)):
            for j in range(0, len(sub_set)):
                if sub_set[i] != sub_set[j]:
                    ni[sub_set[i], sub_set[j]] += 1

    # Save negative graph data to file.
    with open('./NI_matrix_BIG.txt', 'w+') as f:
        for i in xrange(columns):
            line = ""
            for j in xrange(columns):
                line += (int(ni[i, j]).__str__()) + '\t'
            f.write(line[:-1] + '\n')
...
def get_sub_negative_graph(rating_mat_path, negative_mat_path, user):
    # Load rating matrix data and negative matrix data.
    rating_matrix = np.loadtxt(open(rating_mat_path, 'rb'), dtype=int, delimiter="\t")
    negative_matrix = np.loadtxt(open(negative_mat_path, 'rb'), dtype=int, delimiter="\t")
    print 'Success\t[Loading data from file completed]'

    # Get the given user's negative related items.
    r = rating_matrix[user, :]
    avg = r.sum() / len(r[r != 0])
    sub_set = []
    for i in range(1, rating_matrix.shape[1]):
        if rating_matrix[user, i] < avg and rating_matrix[user, i] != 0:
            sub_set.append(i)
    print 'Success\t[Find ' + len(sub_set).__str__() + ' rated items]'

    # Build the given user's sub-negative graph.
    sub_graph = np.zeros((len(sub_set), rating_matrix.shape[1]))
    for i in range(0, len(sub_set)):
        sub_graph[i, :] = negative_matrix[sub_set[i], :]
    ...
    for i in range(0, len(sub_set)):
        ...
        for to_id in range(1, rating_matrix.shape[1]):
            if negative_matrix[from_id, to_id] != 0:
                candidates.append(Edge(from_id, to_id, negative_matrix[from_id, to_id]))
    ...
    with open('./Negative_candidate_edges_BIG_' + str(user) + '.txt', 'w+') as f:
        for i in candidates:
...
def make_recommendation(rating_mat_path, negative_candidates_mat_path, user, k):
    # Load rating matrix data and candidates matrix data.
    rating_matrix = np.loadtxt(open(rating_mat_path, 'rb'), dtype=int, delimiter="\t")
    negative_candidates_matrix = np.loadtxt(open(negative_candidates_mat_path, 'rb'), dtype=int, delimiter="\t")
    ...
    for i in xrange(negative_candidates_matrix.shape[0]):
        from_id = negative_candidates_matrix[i, 0]
        to_id = negative_candidates_matrix[i, 1]
        weight = negative_candidates_matrix[i, 2]
    ...
    cnt = 0
    with open('./Negative_recommended_items_BIG_' + str(user) + '.txt', 'w+') as f:
        for i in candidates:
            if cnt >= k:
                break
...
def validate_prediction(test_bench_path, prediction_item_path, user):
    ...
    for i in range(0, len(test_bench)):
        if prediction.__contains__(test_bench[i, 1]):
            ...
            if test_bench[i, 2] + prediction.get(test_bench[i, 1]) >= 2 and test_bench[i, 2] + prediction.get(test_bench[i, 1]) <= 4:
...
def automatically_recommend(user, k):
    ...
    if not os.path.isfile('./NI_matrix_BIG.txt'):
        if not os.path.isfile('./rating_dat_BIG.txt'):
            print 'Error\t[rating_dat_BIG.txt not found]'
            return
        else:
            calc_negative_mat_from_rating_mat('./rating_dat_BIG.txt')
    print 'Success\t[Got negative matrix data]'

    if not os.path.isfile('./Negative_candidate_edges_BIG_' + str(user) + '.txt'):
        get_sub_negative_graph('./rating_dat_BIG.txt', './NI_matrix_BIG.txt', user)

    if not os.path.isfile('./Negative_recommended_items_BIG_' + str(user) + '.txt'):
        make_recommendation('./rating_dat_BIG.txt', './Negative_candidate_edges_BIG_' + str(user) + '.txt', user, k)

    validate_prediction('./u1_BIG.test', './Negative_recommended_items_BIG_' + str(user) + '.txt', user)

if __name__ == "__main__":
    #convert_to_rating_mat('./u1_BIG.base')
    #calc_negative_mat_from_rating_mat('./rating_dat_BIG.txt')
    #get_sub_negative_graph('./rating_dat_BIG.txt', './NI_matrix_BIG.txt', 1)
    #make_recommendation('./rating_dat_BIG.txt','./Negative_candidate_edges_BIG_1.txt',1, 100)
    #validate_prediction('./u1_BIG.test','./Negative_recommended_items_BIG_1.txt',1)
    automatically_recommend(42, 10)